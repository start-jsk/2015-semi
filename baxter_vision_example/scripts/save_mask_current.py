...
def callback_bg_mask(data):
    ...
    path = rospack.get_path('baxter_vision_example') + u"/data/bg_mask_cur.jpg" 
...
def callback_bg(data):
    bridge = cv_bridge.CvBridge()
    rospack = rospkg.RosPack()
    path = rospack.get_path('baxter_vision_example') + u"/data/bg_cur.jpg" 
    img = bridge.imgmsg_to_cv2(data,data.encoding)
    cv2.imwrite(path,img)
...
def callback_fg_mask(data):
    bridge = cv_bridge.CvBridge()
    rospack = rospkg.RosPack()
    path = rospack.get_path('baxter_vision_example') + u"/data/fg_mask_cur.jpg" 
    img = bridge.imgmsg_to_cv2(data,data.encoding)
    cv2.imwrite(path,img)
...
def callback_fg(data):
    bridge = cv_bridge.CvBridge()
    rospack = rospkg.RosPack()
    path = rospack.get_path('baxter_vision_example') + u"/data/fg_cur.jpg" 
    img = bridge.imgmsg_to_cv2(data,data.encoding)
    cv2.imwrite(path,img)
